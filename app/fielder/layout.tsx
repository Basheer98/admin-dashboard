...
export default async function FielderLayout({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 sm:flex-row">
      <aside className="flex w-full flex-col border-b border-slate-200 bg-slate-900 px-4 py-6 sm:w-56 sm:border-b-0 sm:border-r sm:border-slate-200">
        ...
        <form method="POST" action="/api/auth/logout" className="mt-auto pt-6">
          <button
            type="submit"
            className="w-full rounded-lg border border-slate-600 px-3 py-2.5 text-left text-sm font-medium text-slate-400 hover:bg-white/5 hover:text-white"
          >
            Log out
          </button>
        </form>
